...
class MyAppSupabaseUser extends BaseAuthUser {
  MyAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> myAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MyAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
